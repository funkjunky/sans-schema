...
import forEachM2mRelation from './forEachM2mRelation'; // const flattenUsers = flatten('users')
  ...
  const flattenModel = (modelName, model) => {
    ...
    Object.keys(model).forEach(key => {
      ...
      if (expandable(model, key, modelName, oneToOne)) {
        const mappedKey = getPropertyTree(keyToModel, key, modelName, key); // if member is an array, then extract all the models into flattened data

        if (Array.isArray(model[key])) {
          const m2mRelation = Object.entries(manyToMany).find(([m2mKey, relations]) => Object.values(relations).includes(key));

          if (m2mRelation) {
            const m2mKey = m2mRelation[0]; // Note: this is a bit of a mind fuck... we want the key for the value that is NOT the key we provided. Because m2m key points to the referenced key alias.
            // persons: favourites
            // hockeyTeams: fans
            ...
            model[key].forEach(({
              ...
              if (!getPropertyTree(result[m2mKey][key], [], id, m2mModelAlias).find(({
                id
              }) => id === model.id)) {
            ...
            result[modelName][model.id][key] = model[key].map(({
              id
            }) => ({
              id
            }));
