...
import pluralize, { singulize } from './pluralize'; // Used to remove / nullify related members
...
export default ((modelName, model, state, config = {}) => {
  ...
  let result = {}; // manyToMany (m2m may not and often isn't in the object itsself, unless expanded)
  ...
  forEachM2mRelation(modelName, manyToMany, (m2mModelKey, otherKey) => {
    ...
    Object.values(state[m2mModelKey][otherKey]).forEach(m => {
      let modelNameAlias = manyToMany[m2mModelKey][otherKey];
      let keyModel = result[m2mModelKey][otherKey];
      keyModel[m.id] = {
        id: m.id,
        [modelNameAlias]: state[m2mModelKey][otherKey][m.id][modelNameAlias].filter(({
          id
        }) => id !== model.id)
      };
    });
  ...
  Object.keys(fullModel).forEach(key => {
    let mappedKey = getPropertyTree(keyToModel, key, modelName, key); // if somehow a m2m key got into the object, then ignore it. We handle m2m automatically above.
    ...
    if (Array.isArray(fullModel[key])) {
      ...
      const stateEntity = state[pluralizedMappedKey][fullModel[key].id]; // filter out the referenced model
