...
import forEachM2mRelation from './forEachM2mRelation'; // Function that expands things based on keyToModel
...
export default ((modelName, model, state, deepness = 2, config = {}) => {
  const {
    ...
    oneToOne = {},
    manyToMany = {}
  } = config; //TODO: defining _expandModel function every call seems ineffecient
  ...
  const _expandModel = (modelName, model, deepness) => {
    ...
    Object.keys(newModel).forEach(key => {
      if (!expandable(storeModel, key, modelName, oneToOne)) return; //get the model name for the key, for example work_commit => commits
    ...
    forEachM2mRelation(modelName, manyToMany, (m2mKey, key, origKey) => {
      const modelNameKey = Object.entries(manyToMany[m2mKey]).find(([k, v]) => k !== modelName)[1]; // If an m2mKey exists, but is empty, then you can end up with one side having zero relations. This check is for that.
